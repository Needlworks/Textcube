...
class POD extends DBQuery {
    ...
    function POD($domain = null, $type = null, $prefix = '') {
        ...
        if ($type != null) {
            $this->_prototype = $this->_type = $type;
        } else {
            if (isset($this->prototype)) {
                $this->_prototype = $this->_type = $this->prototype;
            }
        }
        if (!empty($prefix)) {
            $this->_tablePrefix = $prefix;
        } else {
            if (isset($_pod_setting['tablePrefix'])) {
                $this->_tablePrefix = $_pod_setting['tablePrefix'];
            } else {
                $this->_tablePrefix = '';
            }
        }
        if (isset($_pod_setting['DBMS'])) {
            $this->_DBMS = $_pod_setting['DBMS'];
        }
        $this->reset();
    ...
    function define($rewriteTreeOnly = false) {
        ...
        if (empty($this->_version) || empty($this->_prototype)) {
            return $this->_false();
        }
        ...
        foreach ($this->_queue[$this->domain()][$this->type()]['instances'] as $instance => $options) {
            ...
            if ($options['attribute'] != null) {
                $clause .= ' ' . $options['attribute'];
            }
            if ($options['additional'] != null) {
                $clause .= ' ' . $options['additional'];
            }
            $clause .= ',';
        }
...
//	}
    ...
    function superClass() {
        if ($this->type() == null) {
            ...
            return $this->_structure[$this->domain()][$this->type()]['superclasses'];
        }
    }
    ...
    function subClass() {
        if ($this->type() == null) {
            ...
            return $this->_structure[$this->domain()][$this->type()]['subclasses'];
        }
    }
    ...
    function version($version = null) {
        ...
        if ($this->type() == null) {
            ...
            return $this->_structure[$this->domain()][$this->type()]['version'];
        }
    }
    ...
    function setInstance($instance, $attribute = null, $additional = null) {
        ...
        if ($attribute != null) {
            $this->_queue[$this->domain()][$this->type()]['instances'][$instance]['attribute'] = $attribute;
        }
        if ($additional != null) {
            $this->_queue[$this->domain()][$this->type()]['instances'][$instance]['additional'] = $additional;
        }
    }
    ...
    function relation($type, $field, $matches) {
        ...
        if ($this->_queue[$this->domain()][$type] == null) {
            return $this->_false();
        }
        if ($this->_queue[$this->domain()][$this->type()] == null) {
            return $this->_false();
        }
        if ($this->_queue[$this->domain()][$type]['instances'][$field] === null) {
            return $this->_false();
        }
        if ($this->_queue[$this->domain()][$this->type()]['instances'][$matches] == null) {
            return $this->_false();
        }
    ...
    function instance($instances) {
        if (func_num_args() == 1) {
            ...
            if (array_key_exists($values, $this->_instances)) {
    ...
    function valueCardinality($instance) {
        if (empty($this->_repository) || empty($this->_repository[0][$instance])) {
            return null;
        }
    ...
    function allValues($instance = null) {
        ...
        foreach ($this->_repository as $repos) {
            if ($repos[$instance] == null && $this->aliases[$instance] != null) {
                ...
                array_push($column, $repos[$instance]);
            }
        }
        return $column;
    }
    ...
    function restrict($type, $condition = null) {
        ...
        if ($this->_references[$type] === true || empty($this->_references[$type])) {
            $item = array();
        } else {
            $item = $this->_references[$type];
        }
        foreach (explode(',', $condition) as $cond) {
            array_push($item, $cond);
        }
...
//		if(!$this->_validateInstance($instance)) return $this->_false();
    ...
    function _loadStructure() {
        if (empty($this->_structure)) {
            ...
            if (!empty($structure)) {
                $this->_structure = unserialize($structure);
            } else {
    ...
    function _clause() {
        $condition = '';
        $baseMarkup = '';
        if (!empty($this->_mergeWith) || !empty($this->_references)) {
            $baseMarkup = $this->type() . '.';
        }
        // Value restriction
    ...
    function _validateInstance($instance) {
        if ($this->_structure[$this->domain()][$this->type()]['instances'][$instance] == null) {
            return $this->_false();
        }
        if ($this->_repository[0][$instance] == null) {
            return $this->_false();
        }
        return true;
    }
    ...
    function _validateType($type) {
        if ($this->_structure[$this->domain()][$type] == null) {
            return $this->_false();
        }
        return true;
    }
    ...
    function _alterStructure() {
        if ($this->_version <= $this->_structure[$this->domain()][$this->type()]['version']) {
            return $this->_false();
        }
