...
class Page {
    ...
    function open($filter = '', $fields = '*', $sort = 'published DESC') {
        ...
        $this->close();
        $this->_result = POD::query("SELECT $fields FROM {$database['prefix']}Entries WHERE blogid = " . getBlogId() . " AND draft = 0 AND category = -3 $filter $sort");
        if ($this->_result) {
    ...
    function remove($id) {
        ...
        $result = POD::query("DELETE FROM {$database['prefix']}Entries WHERE blogid = " . getBlogId() . " AND category = -3 AND id = $id");
...
//			$checkSlogan = POD::escapeString($this->slogan);
    ...
    function doesExist($id) {
        ...
        return POD::queryExistence("SELECT id FROM {$database['prefix']}Entries WHERE blogid = " . getBlogId() . " AND id = $id AND category = -3 AND draft = 0");
    }
