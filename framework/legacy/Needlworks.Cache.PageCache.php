...
class pageCache extends Singleton {
    ...
    public function purge() {
        ...
        if (
            ...
            if (file_exists($this->absoluteFilePathGuest)) {
    ...
    private function getPageCacheLog() {
        ...
        if (!is_null($result)) {
            ...
            if (doesHaveOwnership()) {
                $this->dbContents = isset($this->_dbContents['owner']) ? $this->_dbContents['owner'] : null;
            } else {
                $this->dbContents = isset($this->_dbContents['user']) ? $this->_dbContents['user'] : null;
            }
        } else {
            return false;
        }
...
class globalCacheStorage extends pageCache {
    ...
    function getContent($name) {
        if (empty($this->_gCacheStorage)) {
            $this->load();
        }
        if (isset($this->_gCacheStorage[$this->_gBlogId][$name])) {
            return $this->_gCacheStorage[$this->_gBlogId][$name];
        } else {
            return null;
        }
    }
    ...
    function setContent($name, $value) {
        if (isset($this->_gCacheStorage[$this->_gBlogId][$name]) && $this->_gCacheStorage[$this->_gBlogId][$name] == $value) {
            return true;
        } else {
