<?php
class Model_SubscriptionStatistics {
	...
	function open($filter = '', $fields = '*', $sort = 'subscribed DESC') {
		...
		if (!empty($sort))
			$sort = 'ORDER BY ' . $sort;
		$this->close();
		$this->_result = Data_IAdapter::query("SELECT $fields FROM {$database['prefix']}SubscriptionStatistics WHERE blogid = ".getBlogId()." $filter $sort");
		if ($this->_result) {
	...
	function _buildQuery() {
		...
		$query = new Data_table($database['prefix'] . 'SubscriptionStatistics');
