...
final class Model_Config extends Singleton {
    ...
    private function __basicConfigLoader($id) {
        ...
        if (file_exists(ROOT . '/framework/id/load')) {
            $id = trim(file_get_contents(ROOT . '/framework/id/load'));
        }
        require_once(ROOT . '/framework/id/' . $id . '/config.default.php');    // Loading default configuration
        if (file_exists(__TEXTCUBE_CONFIG_FILE__)) {
            @include(__TEXTCUBE_CONFIG_FILE__);
        ...
        if (!isset($service['session_cookie_path'])) {
        ...
        if (isset($service['dbms'])) {
            if ($service['dbms'] == 'mysql' && class_exists('mysqli')) {
                $service['dbms'] = 'mysqli';
            }
        }
