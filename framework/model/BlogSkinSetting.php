...
class Model_BlogSkinSetting {
	...
	function load($fields = '*') {
		global $database;
		$this->reset();
		if ($result = Data_IAdapter::query("SELECT $fields FROM {$database['prefix']}SkinSettings WHERE blogid = ".getBlogId())) {
	...
	function save() {
		...
		$query = new Data_Table($database['prefix'] . 'SkinSettings');
