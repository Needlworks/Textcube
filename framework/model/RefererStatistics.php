...
class Model_RefererStatistics {
	...
	function open($filter = '', $fields = '*', $sort = 'count DESC') {
		...
		if (!empty($sort))
			$sort = 'ORDER BY ' . $sort;
		$this->close();
		$this->_result = Data_IAdapter::query("SELECT $fields FROM {$database['prefix']}RefererStatistics WHERE blogid = ".getBlogId()." $filter $sort");
		if ($this->_result) {
	...
	function _buildQuery() {
		...
		$query = new Data_Table($database['prefix'] . 'RefererStatistics');
