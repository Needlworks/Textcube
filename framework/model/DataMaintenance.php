...
class Model_DataMaintenance {
	function removeAll($removeAttachments = true) {
		...
		Data_IAdapter::query("DELETE FROM {$database['prefix']}CommentsNotified WHERE blogid = $blogid");
		...
		//Data_IAdapter::query("DELETE FROM {$database['prefix']}UserSettings WHERE user = $blogid");
		...
		Data_IAdapter::query("DELETE FROM {$database['prefix']}FeedGroups WHERE blogid = $blogid AND id <> 0");
		...
		if ($removeAttachments) {
			...
			Data_IAdapter::query("UPDATE {$database['prefix']}BlogSettings SET logo = '' WHERE blogid = $blogid");
		}
	}
}
?>
