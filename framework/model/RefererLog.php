...
class Model_RefererLog {
	...
	function open($filter = '', $fields = '*', $sort = 'referred DESC') {
		...
		if (!empty($sort))
			$sort = 'ORDER BY ' . $sort;
		$this->close();
		$this->_result = Data_IAdapter::query("SELECT $fields FROM {$database['prefix']}RefererLogs WHERE blogid = ".getBlogId()." $filter $sort");
		if ($this->_result) {
	...
	function _buildQuery() {
		global $database;
		$query = new Data_Table($database['prefix'] . 'RefererLogs');
