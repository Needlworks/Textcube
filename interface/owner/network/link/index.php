							...
							//<![CDATA[
								function deleteLink(id) {
									if (!confirm("<?php echo _t('링크를 삭제하시겠습니까?');?>"))
										return;
									var request = new HTTPRequest("GET", "<?php echo $context->getProperty('uri.blog');?>/owner/network/link/delete/" + id);
								...
								function setLinkVisibility(id, command) {
									var request = new HTTPRequest("POST", "<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/" + id);
									request.onSuccess = function () {
										PM.removeRequest(this);
										...
										switch(visibility) {
											...
											case 0:
												...
												document.getElementById("protectedIcon_" + id).innerHTML = '<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/' + id + '?command=protect" onclick="setLinkVisibility('+id+', 1); return false;" title="<?php echo _t('현재 상태를 보호로 전환합니다.');?>"><span class="text"><?php echo _t('보호');?><\/span><\/a>';
												document.getElementById("publicIcon_" + id).innerHTML = '<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/' + id + '?command=public" onclick="setLinkVisibility('+id+', 2); return false;" title="<?php echo _t('현재 상태를 공개로 전환합니다.');?>"><span class="text"><?php echo _t('공개');?><\/span><\/a>';
												break;
											case 1:
												...
												document.getElementById("privateIcon_" + id).innerHTML = '<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/' + id + '?command=private" onclick="setLinkVisibility('+id+', 0); return false;" title="<?php echo _t('현재 상태를 비공개로 전환합니다.');?>"><span class="text"><?php echo _t('비공개');?><\/span><\/a>';
...
?>
										<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/categoryEdit/<?php echo $link['category'];?>" title="<?php echo _t('이 카테고리 이름을 수정합니다. 같은 카테고리의 모든 링크의 카테고리 이름이 함께 바뀝니다.');?>"<?php echo (isset($link['categoryName']) ? '' : ' class="uncategorized"');?>><?php echo (isset($link['categoryName']) ? htmlspecialchars($link['categoryName']) : _t('분류 없음'));?></a>
<?php
} else {
?>
...
?>
										...
										<td class="homepage"><a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/edit/<?php echo $link['id'];?>" title="<?php echo _t('이 링크 정보를 수정합니다.');?>"><?php echo htmlspecialchars($link['name']);?></a></td>
...
?>
												<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/<?php echo $link['id'];?>?command=protect" onclick="setLinkVisibility(<?php echo $link['id'];?>, 0); return false;" title="<?php echo _t('현재 상태를 비공개로 전환합니다.');?>"><span class="text"><?php echo _t('비공개');?></span></a>
<?php
	} else {
?>
...
?>
												<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/<?php echo $link['id'];?>?command=protect" onclick="setLinkVisibility(<?php echo $link['id'];?>, 1); return false;" title="<?php echo _t('현재 상태를 보호로 전환합니다.');?>"><span class="text"><?php echo _t('보호');?></span></a>
<?php
	} else {
?>
...
?>
												<a href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/visibility/<?php echo $link['id'];?>?command=public" onclick="setLinkVisibility(<?php echo $link['id'];?>, 2); return false;" title="<?php echo _t('현재 상태를 공개로 전환합니다.');?>"><span class="text"><?php echo _t('공개');?></span></a>
<?php
	} else {
?>
...
?>
										...
										<td class="edit"><a class="edit-button button" href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/edit/<?php echo $link['id'];?>" title="<?php echo _t('링크 정보를 수정합니다.');?>"><span><?php echo _t('수정');?></span></a></td>
										<td class="delete"><a class="delete-button button" href="<?php echo $context->getProperty('uri.blog');?>/owner/network/link/delete/<?php echo $link['id'];?>" onclick="deleteLink(<?php echo $link['id'];?>); return false;" title="<?php echo _t('링크 정보를 삭제합니다.');?>"><span class="text"><?php echo _t('삭제');?></span></a></td>
									</tr>
<?php
}
