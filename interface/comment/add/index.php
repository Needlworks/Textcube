...
if(!Validator::validate($IV))
	Utils_Respond::PrintResult(array('error' => 1, 'description' => 'Illegal parameters'));
requireStrictRoute();
...
if (!doesHaveMembership() && !doesHaveOwnership() && $userName == '') {
	...
	if (in_array($result, array("ip", "name", "homepage", "comment", "openidonly", "etc"))) {
		...
		if(!$comment['secret']) {
			if($row = Data_IAdapter::queryRow("SELECT * FROM {$database['prefix']}Entries WHERE blogid = $blogid AND id = $entryId AND draft = 0 AND visibility = 3 AND acceptComment = 1"))
