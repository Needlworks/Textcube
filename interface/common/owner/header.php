...
if (isset($urlFragments[1])) {
    $blogMenu['topMenu'] = $urlFragments[1];
}
if (isset($urlFragments[2])) {
    $blogMenu['contentMenu'] = $urlFragments[2];
} else {
    $blogMenu['contentMenu'] = $urlFragments[1];
}
if (isset($urlFragments[3])) {
    $blogMenu['contentMenu'] .= $urlFragments[3];
}
...
} elseif (Acl::check('group.administrators')) {
    $blogTopMenuItem = array(
...
if (isset($blogMenu['topMenu']) && $blogMenu['topMenu'] == 'data') {
...
} elseif ($blogMenu['topMenu'] == 'entry' && ($blogMenu['contentMenu'] == 'post' || $blogMenu['contentMenu'] == 'edit')) {
    if (isset($eventMappings['AddPostEditorToolbox'])) {
        foreach ($eventMappings['AddPostEditorToolbox'] as $tempPlugin) {
            array_push($pluginListForCSS, $tempPlugin['plugin']);
        }
    }
} elseif (isset($pluginDir)) {
...
if (isset($blogMenu['topMenu'])) {
    ...
    if (Acl::check('group.administrators')) {
        ...
        if ($context->getProperty('service.reader') == true) {
            array_push($blogContentMenuItem['network'], array('menu' => 'reader', 'title' => _t('바깥 글 읽기'), 'link' => '/owner/network/reader'));
        }
    } else {
        $blogContentMenuItem['network'] = array();
        if ($context->getProperty('service.reader') == true) {
    ...
    if (Acl::check('group.administrators')) {
        ...
        if (Acl::check('group.creators')) {
            array_push($blogContentMenuItem['plugin'], array('menu' => 'tableSetting', 'title' => _t('플러그인 데이터 관리'), 'link' => '/owner/plugin/tableSetting'));
        }
    }
    if (Acl::check('group.administrators')) {
...
?>
	<script src="<?php echo $context->getProperty('service.path');?>/resources/script/polymer/webcomponentsjs/webcomponents-lite.min.js"></script>
	<link rel="import" href="<?php echo $context->getProperty('service.path');?>/resources/script/polymer/index.imports.html">
	<link rel="import" href="<?php echo $context->getProperty('service.path').$context->getProperty('panel.skin');?>/style.html">
<?php
// common CSS.
foreach ($blogMenu['loadCSS'] as $loadCSS) {
    ?>
	<link rel="stylesheet" type="text/css" media="screen" href="<?php echo $context->getProperty('service.path').$context->getProperty('panel.skin');
    ?>/<?php echo $loadCSS;
    ?>.css" />
<?php
...
foreach ($pluginListForCSS as $tempPluginDir) {
    if (isset($tempPluginDir) && file_exists(ROOT."/plugins/$tempPluginDir/plugin-main.css")) {
        ?>
	<link rel="stylesheet" type="text/css" href="<?php echo $context->getProperty('service.path');
        ?>/plugins/<?php echo $tempPluginDir;
        ?>/plugin-main.css" />
<?php
...
if (!in_array($blogMenu['contentMenu'], array('post', 'edit'))) {
    ?>
<!--		<script type="text/javascript" src="<?php echo $context->getProperty('service.path');
    ?>/resources/script/jquery/jquery-ui-1.8.7.custom.min.js"></script>-->
<?php

}
if (file_exists(ROOT.$context->getProperty('panel.skin').'/custom.js')) {
    ?>
	<script type="text/javascript" src="<?php echo $context->getProperty('service.path').$context->getProperty('panel.skin');
    ?>/custom.js"></script>
<?php
...
<?php
        ...
        ?>
	<script type="text/javascript" src="<?php echo $context->getProperty('service.path');
        ?>/resources/script/<?php echo $context->getProperty('service.admin_script');
        ?>"></script>
<?php
...
<body id="body-<?php echo $blogMenu['topMenu'];?>">
	...
	<app-drawer swipe-open class="drawer-menu">
		<app-header id="menu-textcube" waterfall fixed>
			<div id="menu-textcube"><a href="<?php echo $context->getProperty('uri.blog').'/owner/center/dashboard';?>" title="<?php echo _t('센터로 이동합니다.');?>"><span class="text"><?php echo _t('텍스트큐브');?></span></a></div>
		</app-header>
		...
		<?php
            ...
            foreach ($blogContentMenuItem[$blogMenu['topMenu']] as &$contentMenuItem) {
                ...
                if ($contentMenuItem['menu'] == 'divider') {
                    ?>
								<li class="divider"><span class="divider"><?php echo $contentMenuItem['title'];
                    ?></span><?php

                } else {
                    ?>
								<li id="sub-menu-<?php echo $PostIdStr;
                    ?>"<?php echo
                                ($blogMenu['contentMenu'] == $contentMenuItem['menu'] ||
                                    ...
                                    ($blogMenu['contentMenu'] == 'edit' && $contentMenuItem['menu'] == 'post')) ? " class=\"selected{$firstChildClass}\"" : ($firstChildClass ? " class=\"$firstChildClass\"" : '');
                    ?>><?php
                    if ($contentMenuItem['menu'] == 'divider') {
                        ?><span class="divider"><?php echo $contentMenuItem['title'];
                        ?></span><?php

                    } else {
                        ?><a title="<?php echo $contentMenuItem['title'];
                        ?>" href="<?php
                        ...
                        ?>"><span class="text"><?php echo $contentMenuItem['title'];
                        ?></span></a><?php
        ...
        ?>
				...
				<?php
                                if ('single' != $service['type']) {
                                    ?>
								<label for="blog-list"><?php echo _t('현재 블로그');
                                    ?></label>
	...
	<app-header-layout main id="main-panel">
		<app-header id="main-toolbar" condenses reveals
								effects="waterfall parallax-background" style="height:35px;"
								effects-config='{"resize-snapped-title": {"startsAt": 0.8, "duration": "100ms"}, "parallax-background": {"scalar": 0.5}}'>
				<app-toolbar primary style="height:35px;" class="bar" id="main-menu">
    ...
    foreach ($blogTopMenuItem as $menuItem) {
        ?>
							<li id="menu-<?php echo $menuItem['menu'];
        ?>"<?php echo $menuItem['menu'] == $blogMenu['topMenu'] ? ' class="selected"' : '';
        ?>>
								<a href="<?php echo $context->getProperty('uri.blog').$menuItem['link'];
        ?>" class="menu-name"><span><?php echo $menuItem['title'];
        ?></span></a>
								<ul id="submenu-<?php echo $menuItem['menu'];
        ?>" class="sub-menu">
	<?php
        $firstChildClass = ' firstChild';
    ...
    /*	if (isset($_POST['category'])) $currentCategory = $_POST['category'];
        ...
        if (isset($_POST['status'])) {
            ...
            } elseif ($blogMenu['contentMenu'] == 'trackback') {
                $blogMenu['contentMenu'] = $blogMenu['contentMenu'].$_POST['status'];
            }
        } elseif (in_array($blogMenu['contentMenu'], array('trashcomment', 'trashtrackback'))) {
            $blogMenu['contentMenu'] = 'trash';
        } elseif (in_array($blogMenu['contentMenu'], array('linkadd', 'linkedit', 'linkcategoryEdit', 'xfn'))) {
    ...
    //		$blogMenu['contentMenu'] = 'widget';
        foreach ($blogContentMenuItem[$menuItem['menu']] as &$contentMenuItem) {
            ...
            if ($contentMenuItem['menu'] == 'divider') {
                ...
                ?>
									<li id="main-sub-menu-<?php echo $PostIdStr;
                ?>"<?php echo
        ...
        ($blogMenu['contentMenu'] == 'edit' && $contentMenuItem['menu'] == 'post')) ?
            " class=\"selected{$firstChildClass}\"" : ($firstChildClass ? " class=\"$firstChildClass\"" : '');
                ?>><?php
    ...
    ?>
							<li id="action-move-to-control-panel"><a href="<?php echo $context->getProperty('uri.blog').'/control';
    ?>" title="<?php echo _t('관리 패널로 이동합니다.');
    ?>"><span class="text"><?php echo _t('텍스트큐브 관리');
    ?></span></a></li>
			<?php
} ?>
