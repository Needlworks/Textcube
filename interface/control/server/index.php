...
?>
						...
						//<![CDATA[
							...
							function setConfigFile() {
								...
								if(document.getElementById('usePageCache').checked) usePageCache = true;
								else usePageCache = false;
								if(document.getElementById('useCodeCache').checked) useCodeCache = true;
								else useCodeCache = false;
								if(document.getElementById('useSkinCache').checked) useSkinCache = true;
								else useSkinCache = false;
								if(document.getElementById('useMemcached').checked) useMemcached = true;
								else useMemcached = false;
								if(document.getElementById('useExternalResource').checked) useExternalResource = true;
								else useExternalResource = false;
								if(document.getElementById('useReader').checked) useReader = true;
								else useReader = false;
								if(document.getElementById('useNumericRSS').checked) useNumericRSS = true;
								else useNumericRSS = false;
								if(document.getElementById('useEncodedURL').checked) useEncodedURL = true;
								else useEncodedURL = false;
								if(document.getElementById('allowBlogVisibilitySetting').checked) allowBlogVisibilitySetting = true;
								else allowBlogVisibilitySetting = false;
								if(document.getElementById('requireLogin').checked) requireLogin = true;
								else requireLogin = false;
								if(document.getElementById('flashClipboardPoter').checked) flashClipboardPoter = true;
								else flashClipboardPoter = false;
								if(document.getElementById('flashUploader').checked) flashUploader = true;
								else flashUploader = false;
								if(document.getElementById('useDebugMode').checked) useDebugMode = true;
								else useDebugMode = false;
								if(document.getElementById('useSessionDebugMode').checked) useSessionDebugMode = true;
								else useSessionDebugMode = false;
								if(document.getElementById('useRewriteDebugMode').checked) useRewriteDebugMode = true;
								else useRewriteDebugMode = false;
								param = '';
								...
								param += 'autologinTimeout='+getEncodedValueById('autologinTimeout') +'&';
							...
							<div class="data-inbox">
								<form id="configSetting" class="section" method="post" action="<?php echo $context->getProperty('uri.blog');?>/control/server/config" enctype="application/x-www-form-urlencoded">
									<fieldset id="cache-container" class="container">
										...
										<dl id="autologin-timeout-line" class="line">
											<dt><span class="label"><?php echo _t('자동 로그인 타임아웃');?></span></dt>
											<dd>
												<input id="autologinTimeout" type="text" class="input-text" name="autologinTimeout" size="11" value="<?php echo $service['autologinTimeout'];?>" />
												<label for="autologinTimeout"><?php echo _t('자동 로그인 세션 타임 아웃까지의 시간을 설정합니다. 단위는 초입니다.');?></label>
											</dd>
										</dl>
...
<label for="encoding"><?php echo _t('이 서비스의 기본 인코딩을 설정합니다.');?></label>
									...
									<fieldset id="option-container" class="container">
										...
										<dl id="blog-service-visibility-line" class="line">
											...
											<dd>
												<input type="checkbox" id="requireLogin" class="checkbox" name="requireLogin"<?php echo isset($service['requirelogin']) && !empty($service['requirelogin']) ? ' checked="checked"' : '';?> /><label for="requireLogin"><?php echo _t('다중 블로그 모드에서 블로그 회원만 다른 블로그를 볼 수 있도록 합니다. 비공개 블로그 커뮤니티에 유용합니다.');?></label>
											</dd>
										</dl>
								...
								<form class="section" method="post" action="<?php echo $context->getProperty('uri.blog');?>/control/server/mailhost">
									<dl>
										...
										<dd>
											...
											<div class="line">
												<input id="useCustomSMTP" type="checkbox" class="checkbox" name="useCustomSMTP" value="1" <?php if( $context->getProperty('service.useCustomSMTP',0)) { echo "checked='checked'"; } ?> />
												<label for="useCustomSMTP"><?php echo _t('메일서버 지정'); ?></label>
											</div>
											<div class="line">
												<label for="smtpHost"><?php echo _t('메일서버 IP 주소:포트'); ?></label>
												<input id="smtpHost" type="text" class="input-text" name="smtpHost" value="<?php echo $context->getProperty('service.smtpHost', '127.0.0.1' ); ?>" /> :
												<input id="smtpPort" type="text" class="input-text" name="smtpPort" value="<?php echo $context->getProperty('service.smtpPort', 25 );?>" />
											</div>
										</dd>
									</dl>
									<div class="button-box">
