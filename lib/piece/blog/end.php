...
dress('category', getCategoriesView(getCategories($owner), isset($category) ? $category : true, getCategoriesSkin()), $view);
dress('category_list', getCategoriesView(getCategories($owner), isset($category) ? $category : true, getCategoriesSkin(), true), $view);
...
if (sizeof($notices) > 0) {
	...
	foreach ($notices as $notice) {
		...
		dress('notice_rep_title', htmlspecialchars(fireEvent('ViewNoticeTitle', UTF8::lessenAsEm($notice['title'], $skinSetting['recentNoticeLength']), $entry['id'])), $itemView);
