...
//			'href="'.$defaultURL.'/rss/comment/'.$entry['id'].'" />'.CRLF;
	...
	foreach ($entries as $entry) {
		...
		if ($entry['category'] == - 1) { // This is keylog
			...
			dress('article_rep_category_link', empty($entry['category']) ? "$blogURL/category/" : 
($blog['useSloganOnCategory'] ? "$blogURL/category/".URL::encode($entry['categoryLabel'],$service['useEncodedURL']) : "$blogURL/category/".$entry['category']),$entryView);
