	...
	if ($entry['category'] == - 2) {
		...
		if (sizeof($entryTags) > 0) {
			...
			foreach ($entryTags as $entryTag)
				array_push($tags, "<a href=\"$blogURL/tag/" . encodeURL($entryTag['name']) . '"' . (count($entries) == 1 && getUserSettingGlobal('useRelTag', false) ? ' rel="tag"' : '') . '>' . htmlspecialchars($entryTag['name']) . '</a>');
