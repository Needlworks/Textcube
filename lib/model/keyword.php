...
function getKeylog($owner, $keyword) {	
	...
	return fetchQueryAll("SELECT id, title, content, published FROM {$database['prefix']}Entries WHERE owner = $owner AND draft = 0 $visibility AND category >= 0 AND (title LIKE '%$keyword%' OR content LIKE '%$keyword%') ORDER BY published DESC");
}
?>