...
function getStatistics($blogid) {
	...
	$result = DBQuery::query("select visits from {$database['prefix']}BlogStatistics where owner = $blogid");
	if (mysql_num_rows($result) == 1)
		list($stats['total']) = mysql_fetch_array($result);
	$result = DBQuery::query("select visits from {$database['prefix']}DailyStatistics where owner = $blogid and `date` = " . Timestamp::getDate());
	if (mysql_num_rows($result) == 1)
		list($stats['today']) = mysql_fetch_array($result);
	$result = DBQuery::query("select visits from {$database['prefix']}DailyStatistics where owner = $blogid and `date` = " . Timestamp::getDate(time() - 86400));
	if (mysql_num_rows($result) == 1)
...
function getDailyStatistics($period) {
	...
	return DBQuery::queryAll("SELECT date, visits FROM {$database['prefix']}DailyStatistics WHERE owner = $blogid AND LEFT(date, 6) = $period ORDER BY date DESC");
}

function getMonthlyStatistics($blogid) {
	global $database;
	$statistics = array();
	if ($result = DBQuery::query("select left(date, 6) date, sum(visits) visits from {$database['prefix']}DailyStatistics where owner = $blogid group by left(date, 6) order by date desc")) {
...
function getRefererStatistics($blogid) {
	global $database;
	$statistics = array();
	if ($result = DBQuery::query("select host, count from {$database['prefix']}RefererStatistics where owner = $blogid order by count desc limit 20")) {
		while ($record = mysql_fetch_array($result))
			array_push($statistics, $record);
	}
	return $statistics;
...
function getRefererLogsWithPage($page, $count) {  
	global $database;
	return fetchWithPaging("SELECT host, url, referred FROM {$database['prefix']}RefererLogs WHERE owner = ".getBlogId()." ORDER BY referred DESC", $page, $count);  
}  

function getRefererLogs() {
	global $database;
	return DBQuery::queryAll("SELECT host, url, referred FROM {$database['prefix']}RefererLogs WHERE owner = ".getBlogId()." ORDER BY referred DESC LIMIT 1500");
}
...
function updateVisitorStatistics($blogid) {
	...
	if (DBQuery::query("insert into {$database['prefix']}SessionVisits values('$id', '{$_SERVER['REMOTE_ADDR']}', $blogid)") && (mysql_affected_rows() > 0)) {
		DBQuery::query("update {$database['prefix']}BlogStatistics set visits = visits + 1 where owner = $blogid");
		if (mysql_affected_rows() == 0) {
			if (DBQuery::query("update {$database['prefix']}BlogStatistics set visits = visits + 1 where owner = $blogid") || (mysql_affected_rows() == 0))
				DBQuery::query("insert into {$database['prefix']}BlogStatistics values($blogid, 1)");
		}
		$period = Timestamp::getDate();
		DBQuery::query("update {$database['prefix']}DailyStatistics set visits = visits + 1 where owner = $blogid and `date` = $period");
		if (mysql_affected_rows() == 0) {
		...
		if (!empty($_SERVER['HTTP_REFERER'])) {
			...
			if (!empty($referer['host']) && (($referer['host'] != $_SERVER['HTTP_HOST']) || (strncmp($referer['path'], $blogURL, strlen($blogURL)) != 0))) {
				...
				if (!DBQuery::query("update {$database['prefix']}RefererStatistics set count = count + 1 where owner = $blogid and host = '$host'") || (mysql_affected_rows() == 0))
...
function setTotalStatistics($blogid) {
	global $database;
	DBQuery::execute("DELETE FROM {$database['prefix']}DailyStatistics WHERE owner = $blogid");
	$prevCount = DBQuery::queryCell("SELECT visits FROM {$database['prefix']}BlogStatistics WHERE owner = $blogid");
	if ((!is_null($prevCount)) && ($prevCount == 0))
		return true;
	DBQuery::query("update {$database['prefix']}BlogStatistics set visits = 0 where owner = $blogid");
	if (mysql_affected_rows() == 0)
