...
function getTags($entry) {
	...
	$tags = array();
	$result = mysql_query("select * from {$database['prefix']}Tags, {$database['prefix']}TagRelations where id = tag and entry = $entry and owner = $owner group by tag order by name");
	if ($result) {
...
function getRandomTags($owner) {
	...
	if ($skinSetting['tagboxAlign'] == 1) // order by count
		$result = mysql_query("select name, count(*) cnt from {$database['prefix']}Tags, {$database['prefix']}TagRelations where id = tag and owner = $owner GROUP BY tag ORDER BY cnt DESC $aux");
	else if ($skinSetting['tagboxAlign'] == 2) // order by name
		$result = mysql_query("select distinct name from {$database['prefix']}Tags, {$database['prefix']}TagRelations where id = tag and owner = $owner GROUP BY tag ORDER BY name $aux");
	else // random
		$result = mysql_query("select name from {$database['prefix']}Tags, {$database['prefix']}TagRelations where id = tag and owner = $owner GROUP BY tag ORDER BY RAND() $aux");
	if ($result) {
...
function getSiteTags($owner) {
	global $database;
	$names = array();
	$result = mysql_query("select name from {$database['prefix']}Tags t, {$database['prefix']}TagRelations r where t.id = r.tag and r.owner = $owner GROUP BY tag order by name LIMIT 2000"); // TODO: Need paging SiteTags
	if ($result) {
