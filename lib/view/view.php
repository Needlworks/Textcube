...
function getCategoriesView($categories, $selected, $skin, $xhtml = false) {
	...
	foreach ($categories as $category1) {
		...
		foreach ($category1['children'] as $category2) {
			array_push($children, array('id' => $category2['id'], 'label' => $category2['name'], 'value' => $category2[$entriesSign], 'link' => "$blogURL/category/" . escapeURL("{$category1['name']}/{$category2['name']}"), 'children' => array()));
		}
		array_push($tree['children'], array('id' => $category1['id'], 'label' => $category1['name'], 'value' => $category1[$entriesSign], 'link' => "$blogURL/category/" . escapeURL($category1['name']), 'children' => $children));
	}
	ob_start();
...
function getRandomTagsView($tags, & $template) {
	...
	foreach ($tags as $tag) {
		$view = $template;
		dress('tag_link', "$blogURL/tag/" . htmlspecialchars(escapeURL($tag)), $view);
	...
	while ((($start = strpos($view, '[##_')) !== false) && (($end = strpos($view, '_##]', $start + 4)) !== false)) {
		...
		if ($attributes[0] == 'Gallery') {
			...
			if (defined('__TATTERTOOLS_MOBILE__')) {
				...
				$buf .= '<script type="text/javascript">writeCode(getEmbedCode(\'' . $service['path'] . '/script/jukebox/flash/main.swf\',\'' . $width . '\',\'' . $height . '\',\'jukeBox' . $id . 'Flash\',\'#FFFFFF\',"sounds=' . $imgStr . '&amp;autoplay=' . $params['autoplay'] . '&amp;visible=' . $params['visible'] . '&amp;id=' . $id . '","false"))</script>';
	...
	<td>
	  <table width="100%" border="0" cellspacing="0" cellpadding="0">
		  <tr>
			...
			<td align="right" valign="top"><a id="entryPermalink" href="<?=htmlspecialchars($entry['permalink'])?>" target="_blank"><img src="<?=$service['path']?>/image/owner/reader/viewNewwindow.gif" vspace="5" /></a></td>
		  </tr>
		  <tr height="1">
			<td colspan="2" background="<?=$service['path']?>/image/owner/reader/dotline.gif"></td>
		  </tr>
