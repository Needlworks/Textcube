...
function getCommentView($entryId, & $skin) {
	...
	foreach ($comments as $commentItem) {
		...
		foreach (getCommentComments($commentItem['id']) as $commentSubItem) {
			...
			dress($prefix1 . '_rep_desc', fireEvent(($isComment ? 'ViewCommentContent' : 'ViewGuestCommentContent'), nl2br(addLinkSense(stripHTML($commentSubItem['comment'], array('strong', 'em', 'i', 'a', 'b', 'ul', 'li', 'pre', 'hr', 'blockquote', 'img')), ' onclick="return openLinkInNewWindow(this)"')), $commentSubItem), $commentSubItemView);
		...
		dress($prefix1 . '_rep_desc', fireEvent(($isComment ? 'ViewCommentContent' : 'ViewGuestCommentContent'), nl2br(addLinkSense(stripHTML($commentItem['comment'], array('strong', 'em', 'i', 'a', 'b', 'ul', 'li', 'pre', 'hr', 'blockquote', 'img')), ' onclick="return openLinkInNewWindow(this)"')), $commentItem), $commentItemView);
