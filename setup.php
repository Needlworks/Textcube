...
<?php
        ...
        if (file_exists($filename)) {
            if (is_writable($filename)) {
                ...
                $error = 8;
               echo '<li style="color:red">', _t('웹 설정 파일'), ': ', _f('"%1"에 접근할 수 없습니다. 퍼미션을 %2(으)로 수정해 주십시오.', $filename, '0666'), '<br /><span class="instruction">',_f('FTP 프로그램으로 권한을 수정하시거나 다음의 명령을 터미널에 붙여 넣으시면 됩니다 : %1','chmod 0666 '.$filename), '</span></li>';
            }
        }
        ...
        if (file_exists($filename)) {
            if (is_dir($filename) && is_writable($filename))
               echo '<li>', _t('첨부 디렉토리'), ': OK</li>';
                ...
                $error = 12;
               echo '<li style="color:red">', _t('첨부 디렉토리'), ': ', _f('"%1"에 접근할 수 없습니다. 퍼미션을 %2(으)로 수정해 주십시오.', $filename, '0777'), '<br /><span class="instruction">',_f('FTP 프로그램으로 권한을 수정하시거나 다음의 명령을 터미널에 붙여 넣으시면 됩니다 : %1','chmod 0777 '.$filename), '</span></li>';
            }
...
/*        $filename = $root . '/remote';
        ...
        if (is_dir($filename)) {
           ...
           echo '<li>', _t('스킨 디렉토리'), ': OK</li>';
            ...
            $error = 15;
           echo '<li style="color:red">', _t('스킨 디렉토리'), ': ', _f('"%1"에 %2 디렉토리를 생성할 수 없습니다. "%1"의 퍼미션을 %3(으)로 수정해 주십시오.', "$root/skin", 'customize', '0777'), '<br /><span class="instruction">', _f('FTP 프로그램으로 권한을 수정하시거나 다음의 명령을 터미널에 붙여 넣으시면 됩니다 : %1','chmod 0777 '.$root.'/skin'), '</span></li>';
        }
?>
    </ul>
<?php
...
INSERT INTO {$_POST['dbPrefix']}Teamblog VALUES (1, 1, 16, ".Timestamp::getUNIXtime().", 0);
