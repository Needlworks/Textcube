...
$__text['서버의 트래픽을 줄이기 위하여 동작시 필요한 일부 리소스를 공개 텍스트큐브 리소스 저장소(%1)에서 읽어오거나 지정한 서버에서 읽어옵니다.'] = '为了减低服务器流量以节省服务器资源，以公开的 TextCube 服务器(%1) 来存取共享资源。';
$__text['외부 리소스 저장소 경로'] = '外部资源路径';
$__text['외부 리소스를 사용할 경우 리소스 저장소를 임의로 지정할 수 있습니다.'] = '在开启使用外部资源时，任意指定外部资源存取位置。';
$__text['이 값을 지정하지 않고 외부 리소스 사용을 선택할 경우 텍스트큐브 리소스 저장소(%1)를 기본값으로 사용합니다.'] = '在不指定任何外部资源存取位置时，将使用默认 TextCube(%1) 的存取位置。';
...
/* CT_Mobile */
$__text['제목없음'] = '无主旨';
$__text['기존 글을 엽니다.'] = '开启既有文章。';
$__text['새 글을 작성합니다.'] = '制作新的文章。';
...
$__text['블로그에 등록된 플러그인의 설정사항을 복원 및 백업 할수 있습니다. 플러그인을 <b>\'사용중\'</b>으로 해놓은 것들만 백업 대상이 됩니다.<br /> 플러그인 각각의 설정값을 그대로 복원 및 백업 할수 있습니다. 텍스트큐브 v1.5 버전 이상에서만 사용할 수 있고, 복원하시려면 적어도 <b>\'플러그인 설정 관리\'</b> 플러그인은 <br />사용중으로 되어 있어야 합니다. 플러그인 설정 데이터 초기화 하시기 전에 반드시 백업 하시기 바랍니다.'] = '用于恢复博客插件的数据与数据设定、备份插件数据库。仅对于 <b>\'使用中\'</b> 的插件数据做备份与回存动作。<br />分别对于插件的设定与数据库作复原以及回存，需要支持 TextCube v1.5 以上版本的插件才可使用此功能。<br />在设定复原插件设定初始化前，请先确认插件是否支持。';
$__text['플러그인 설정 데이터를 복원합니다'] = '修复插件所使用的数据设定';
$__text['플러그인 설정 데이터 복원 이미지'] = '修复插件所使用的图像设定';
$__text['백업파일을 읽어서 데이터를 복원합니다.<br />백업파일에 첨부파일이 포함되어 있으면 첨부파일도 자동으로 복원됩니다.'] = '正在读取备份文件恢复数据<br />如果备份文件上包含附件时，自动恢复包含的附件。';
$__text['플러그인 설정데이터 복원을 시작합니다'] = '开始复原插件所使用的数据设定';
$__text['서버에 <font color="blue"><em>%1</em></font>에 저장된 플러그인 설정 백업파일이 있습니다.'] = '插件所储存的设定备份文件以储存于服务器 <font color="blue"><em>%1</em></font> 上。';
$__text['(서버에 저장된 백업파일 <a href="#" onclick="downloadBackup();"><font color="#C56D54">다운로드 받기</font></a>)'] = '(伺服器上的備份檔案 <a href="#" onclick="downloadBackup();"><font color="#C56D54">按此下载</font></a>)';
...
$__text['현재의 플러그인 설정 데이터를 백업파일로 보관합니다.<br />첨부파일을 포함시킬 수 있으며, 복원할 경우 자동으로 첨부파일이 처리됩니다.<br />백업파일은 서버에 저장하거나 다운받으실 수 있습니다.'] = '制作储存插件数据设定备份文件。当你包含附件时将在数据复原时一起被重建。此下载将制作插件数据库备份用数据。';
$__text['플러그인 설정 데이터 백업을 시작합니다'] = '插件数据备份设定';
$__text['서버 저장 또는 다운로드를 선택하여 백업하세요.'] = '选择备份文件储存于服务器或下载备份文件';
$__text['(서버 저장은 <b>\'%1/cache/backup/%2_plugins_set_backup.xml\'</b> 경로에 저장 됩니다.)'] = '备份数据储存于  <b>\'%1/cache/backup/%2_plugins_set_backup.xml\'</b> 此档案中。';
$__text['플러그인 설정 데이터를 초기화 합니다'] = '插件資料設定初始化';
$__text['플러그인 설정 데이터를 모두 초기화 합니다. 사용중인 모든 플러그인을 미사용으로 초기화 한다는 의미입니다.<br />플러그인 설정 데이터의 복원은 백업파일로만 가능하므로 먼저 백업을 하시기 바랍니다.'] = '将所有的插件设定初始化，意指将所有使用中的插件设定为不使用并重新设定。<br />由于会修改设定，使用之前请先将插件数据设定备份。';
