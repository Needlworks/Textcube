			...
			<form method="post" action="">
				...
				<div id="data-outbox">
					<div id="login-box">
			            ...
			            <div id="field-box">
			            	<dl id="email-line">
			            		<dt><label for="loginid"><?=_text('이메일')?></label></dt>
			            	...
			            	<dl id="password-line">
			            		<dt><label for="password"><?=_text('비밀번호')?></label></dt>
							...
							<dl id="checkbox-line">
								<dt><span class="label"><?=_text('선택사항')?></span></dt>
								<dd>
									<div id="email-save"><input type="checkbox" id="save" class="checkbox" name="save"<?=(empty($_COOKIE['TSSESSION_LOGINID']) ? '' : 'checked="checked"')?> /> <label for="save"><?=_text('이메일 저장')?></label></div>
									<?=($showPasswordReset ? '<div id="password_int"><input type="checkbox" class="checkbox" id="reset" name="reset" /> <label for="reset">' . _text('암호 초기화') . '</label></div>'.CRLF : '')?>
								</dd>
							</dl>
							
							<div class="button-box">
								<a class="login-button button" href="#void" onclick="document.forms[0].submit()"><span class="text"><?=_text('로그인')?></span></a>
							</div>
						</div>
