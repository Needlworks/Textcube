...
if ($_REQUEST['type'] == 1) { // type 1, plugin
	...
	$query = "select name from {$database['prefix']}ServiceSettings WHERE value = '" . mysql_real_escape_string($_REQUEST['name']) . "'";
	...
	foreach($plugintablesraw as $dbname) {
		...
		$query = "delete from {$database['prefix']}ServiceSettings WHERE name = '$origname' AND value = '" . mysql_real_escape_string($_REQUEST['name']) . "'";
