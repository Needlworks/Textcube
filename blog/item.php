...
if (isset($_POST['partial'])) {
	...
	if (empty($entries)) {
		...
		dress('article_rep', '<div style="text-align:center;font-size:14px;font-weight:bold;padding-top:50px;margin:50px 0;color:#333;background:url(' . $service['path'] . '/image/warning.gif) no-repeat top center;">' . _t('존재하지 않는 페이지입니다') . '</div>', $view);
	} else {
		require ROOT . '/lib/piece/blog/entries.php';
	}
