...
if (DBQuery::queryCell("DESC {$database['prefix']}Trackbacks owner", 'Key') != 'MUL') { // Since 1.0.2
	$changed = true;
	echo '<li>', _text('트랙백 테이블의 인덱스를 수정합니다.'), ': ';
...
if (!DBQuery::queryExistence("DESC {$database['prefix']}Trackbacks isFiltered")) {
	$changed = true;
	echo '<li>', _text('트랙백 테이블에 광고 및 스팸 분류를 위한 휴지통 필드를 추가합니다.'), ': ';
...
if (DBQuery::queryExistence("DESC {$database['prefix']}Trackbacks sender")) {
	$changed = true;
	echo '<li>', _text('트랙백 테이블의 미사용 필드를 삭제합니다.'), ': ';
...
if (!DBQuery::queryExistence("DESC {$database['prefix']}UserSettings user")) {
	$changed = true;
	echo '<li>', _text('사용자 설정을 위한 테이블을 추가합니다.'), ': ';
	$query = "
...
if ((DBQuery::queryExistence("DESC {$database['prefix']}Personalization owner"))&&(DBQuery::queryExistence("DESC {$database['prefix']}UserSettings user"))) {
	$changed = true;
	echo '<li>', _text('사용자 설정을 위한 테이블로 Personalization 테이블의 정보를 이동합니다.'), ': ';
