...
if ($result && ($row = mysql_fetch_array($result))) {
	...
	$item = array('title' => $row['title'], 'description' => $row['content'], 'link' => "$hostURL$blogURL/{$row['id']}", 'categories' => array(), 'location' => $row['location'], 'pubDate' => Timestamp::getRFC1123GMT($row['published']));
	...
	echo '	<siteURL>', htmlspecialchars("$hostURL$blogURL/"), '</siteURL>', CRLF;
