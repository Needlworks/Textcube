...
function FM_Markdown_format($blogid, $id, $content, $keywords = array(), $useAbsolutePath = false, $bRssMode = false) {
	...
	$view = FM_Markdown_bindAttachments($id, $path, $url, $content, $useAbsolutePath, $bRssMode);
	if (is_array($keywords)) $view = FM_Markdown_bindKeywords($keywords, $view);
	$view = FM_Markdown_bindTags($id, $view);
	return $view;
}
...
function FM_Markdown_bindTags($id, $content) {
...
function FM_Markdown_bindKeywords($keywords, $content) {
...
function FM_Markdown_bindAttachments($entryId, $folderPath, $folderURL, $content, $useAbsolutePath = false, $bRssMode = false) {
		...
		if ($attributes[0] == 'Gallery') {
			...
			if (defined('__TEXTCUBE_MOBILE__')) {
				...
				for ($i = 0; $i < count($images); $i++) {
					if (!empty($images[$i])) {
						if ($i % 2 == 0)
							$buf .= '<div align="center">' . FM_Markdown_getAttachmentBinder($images[$i], '', $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . '</div>';
					...
					if (!empty($images[$i])) {
						if ($i % 2 == 0) {
							...
							if (list($width, $height) = @getimagesize("$folderPath/{$images[$i]}")) {
								...
								$buf .= '<div align="center">' . FM_Markdown_getAttachmentBinder($images[$i], $tempProperty, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . '</div>';
							}
						} else if (strlen($images[$i]) > 0) {
							$buf .= "<div align=\"center\">{$images[$i]}</div>";
						}
			...
			if (defined('__TEXTCUBE_MOBILE__')  || ($bRssMode == true)) {
				...
				for ($i = 0; $i < count($images); $i += 2) {
					if (!empty($images[$i]))
						$buf .= '<div>' . FM_Markdown_getAttachmentBinder($images[$i], '', $folderPath, $folderURL, 1, $useAbsolutePath) . '</div>';
				}
			...
			switch (count($attributes)) {
				case 4:
					list($newProperty, $onclickFlag) = FM_Markdown_createNewProperty($attributes[1], $contentWidth, $attributes[2]);

					if (defined('__TEXTCUBE_MOBILE__')) {
						$buf = '<div>' . FM_Markdown_getAttachmentBinder($attributes[1], $newProperty, $folderPath, $folderURL, 1, $useAbsolutePath) . "</div><div>$attributes[3]</div>";
					} else {
						...
						$buf = $prefix . FM_Markdown_getAttachmentBinder($attributes[1], $newProperty, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode, $onclickFlag) . $caption . '</div>';
					}
					break;
				case 7:
					$eachImageWidth = floor(($contentWidth - 5 * 3) / 2);
					list($newProperty1, $onclickFlag1) = FM_Markdown_createNewProperty($attributes[1], $eachImageWidth, $attributes[2]);
					list($newProperty2, $onclickFlag2) = FM_Markdown_createNewProperty($attributes[4], $eachImageWidth, $attributes[5]);
					if (defined('__TEXTCUBE_MOBILE__')) {
						$buf = '<div>' . FM_Markdown_getAttachmentBinder($attributes[1], $newProperty1, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . "</div><div>$attributes[3]</div>";
						$buf .= '<div>' . FM_Markdown_getAttachmentBinder($attributes[4], $newProperty2, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . "</div><div>$attributes[6]</div>";
					} else {
						...
						$buf = '<div class="imageblock dual" style="text-align: center;"><table cellspacing="5" cellpadding="0" border="0" style="margin: 0 auto;"><tr><td>'
							. FM_Markdown_getAttachmentBinder($attributes[1], $newProperty1, $folderPath, $folderURL, 2, $useAbsolutePath, $bRssMode, $onclickFlag1) . $cap1 . '</td><td>'
							. FM_Markdown_getAttachmentBinder($attributes[4], $newProperty2, $folderPath, $folderURL, 2, $useAbsolutePath, $bRssMode, $onclickFlag2) . $cap2 . '</td></tr></table></div>';
					}
					break;
				case 10:
					...
					list($newProperty1, $onclickFlag1) = FM_Markdown_createNewProperty($attributes[1], $eachImageWidth, $attributes[2]);
					list($newProperty2, $onclickFlag2) = FM_Markdown_createNewProperty($attributes[4], $eachImageWidth, $attributes[5]);
					list($newProperty3, $onclickFlag3) = FM_Markdown_createNewProperty($attributes[7], $eachImageWidth, $attributes[8]);
					if (defined('__TEXTCUBE_MOBILE__')) {
						$buf = '<div>' . FM_Markdown_getAttachmentBinder($attributes[1], $newProperty1, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . "</div><div>$attributes[3]</div>";
						$buf .= '<div>' . FM_Markdown_getAttachmentBinder($attributes[4], $newProperty2, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . "</div><div>$attributes[6]</div>";
						$buf .= '<div>' . FM_Markdown_getAttachmentBinder($attributes[7],$newProperty3, $folderPath, $folderURL, 1, $useAbsolutePath, $bRssMode) . "</div><div>$attributes[9]</div>";
					} else {
						$cap1 = strlen(trim($attributes[3])) > 0 ? '<p class="cap1">' . $attributes[3] . '</p>' : '';
						$cap2 = strlen(trim($attributes[6])) > 0 ? '<p class="cap1">' . $attributes[6] . '</p>' : '';
						...
						$buf = '<div class="imageblock triple" style="text-align: center"><table cellspacing="5" cellpadding="0" border="0" style="margin: 0 auto;"><tr><td>'
							. FM_Markdown_getAttachmentBinder($attributes[1], $newProperty1, $folderPath, $folderURL, 3, $useAbsolutePath, $bRssMode, $onclickFlag1) . $cap1 . '</td><td>'
							. FM_Markdown_getAttachmentBinder($attributes[4], $newProperty2, $folderPath, $folderURL, 3, $useAbsolutePath, $bRssMode, $onclickFlag2) . $cap2 . '</td><td>'
							. FM_Markdown_getAttachmentBinder($attributes[7], $newProperty3, $folderPath, $folderURL, 3, $useAbsolutePath, $bRssMode, $onclickFlag3) . $cap3 . '</td></tr></table></div>';
					}
					break;
...
function FM_Markdown_getAttachmentBinder($filename, $property, $folderPath, $folderURL, $imageBlocks = 1, $useAbsolutePath = false, $bRssMode = false, $onclickFlag=false) {
...
function FM_Markdown_createNewProperty($filename, $imageWidth, $property) {
	$blogid = getBlogId();
