...
function getSubscriptionLogsWithPage($page, $count) {  
	...
	return fetchWithPaging("SELECT ip, host, useragent, referred FROM {$database['prefix']}SubscriptionLogs WHERE owner = $owner ORDER BY referred DESC", $page, $count);  
}  
...
function updateSubscriptionStatistics($target, $mother) {
	...
	$host = mysql_real_escape_string($_SERVER['REMOTE_HOST']);
