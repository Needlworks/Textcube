...
function tinyMCE_handleconfig($configVal) {
    ...
    if (isset($config['editormode']) && $config['editormode'] != 'simple' && $config['editormode'] != 'advanced') {
        return false;
    }
    if (isset($config['paragraphdelim']) && $config['paragraphdelim'] != 'P' && $config['paragraphdelim'] != 'BR') {
        return false;
    }
    return true;
}
...
function tinyMCE_editorinit($editor) {
    ...
    if (empty($config['srctheme'])) {
        $config['srctheme'] = 'default';
    }
    if ($config['srctheme'] == 'default') {
        $config['srctheme'] = 'elegant';
    }
    $config['formatter'] = null;
    if (empty($config['paragraphdelim'])) {
        $config['paragraphdelim'] = 'BR';
    }
