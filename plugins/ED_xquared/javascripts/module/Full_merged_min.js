...
},initFromParams:function(){if(xq.macro.FlashMovieMacro.skipProviderValidation){return true
...
xq.macro.FlashMovieMacro.whiteList={tvpot:/http:\/\/flvs\.daum\.net\/flvPlayer\.swf\?/,youtube:/http:\/\/(?:www\.)?youtube\.com\/v\//,youtube2:/http:\/\/(?:www\.)?youtube\.com\/p\//,pandoratv:/http:\/\/flvr\.pandora\.tv\/flv2pan\/flvmovie\.dll/,pandoratv2:/http:\/\/imgcdn\.pandora\.tv\/gplayer\/pandora\_EGplayer\.swf\?/,mncast:/http:\/\/dory\.mncast\.com\/mncastPlayer\.swf\?/,yahoo:/http:\/\/d\.yimg\.com\//,mgoon:/http:\/\/play\.mgoon\.com\/Video\//,slideshare:/http:\/\/static\.slidesharecdn\.com\/swf\//,vimeo:/http:\/\/(?:www\.)?vimeo\.com\/moogaloop\.swf\?/,storyq:/http:\/\/filefarm\.storyq\.net\/SlideView\.swf\?/,mandki:/http:\/\/www\.mandki\.com\/mandki\/viewer\.swf\?/,andu:/http:\/\/andu\.hanafos\.com\/home\/play\/playgw\.asp\?/,plaync:/http:\/\/static\.plaync\.co\.kr\/plaza\/pcc\/view\/viewskin\_s\.swf\?movie\_id\=/,naver:/http:\/\/serviceapi\.nmv\.naver\.com\/flash\/NFPlayer\.swf\?vid\=/,migame:/http:\/\/free\.migame\.tv\/UploadComponent\/FLVPlayer\/player/,scribd:/http:\/\/d\.scribd\.com\/ScribdViewer\.swf\?/,dailymotion:/http:\/\/(?:www\.)?dailymotion\.com\/swf\//};
xq.macro.FlashMovieMacro.recognize=function(A){if(xq.macro.FlashMovieMacro.skipProviderValidation){return true
}var B=xq.macro.FlashMovieMacro.whiteList;
...
var E=this.editHistory.onCommand();
this._fireOnCurrentContentChanged(this)
}else{alert(A._("Unknown URL pattern"))
}return true
};
...
A.handleInsertIFrame=function(D){this.focus();
var C=this.macroFactory.createMacroFromDefinition({id:"IFrame",params:D});
if(C){var B=C.createPlaceHolderHtml();
this.rdom.insertHtml(B)
}else{alert(A._("Unknown error"))
}};
A.handleIFrame=function(){var B=new xq.ui.FormDialog(this,xq.ui_templates.basicIFrameDialog,function(D){},function(E){this.focus();
if(xq.Browser.isTrident){var D=this.rdom.rng();
D.moveToBookmark(C);
D.select()
}if(!E){return 
}this.handleInsertIFrame(E)
}.bind(this));
if(xq.Browser.isTrident){var C=this.rdom.rng().getBookmark()
}B.show({position:"centerOfEditor"});
return true
