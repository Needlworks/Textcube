...
Versions:
----------------------------------------------------------------------------
* Version 0.9.8a (2006-07-26):
+ Fix
	- index.php 를 가볍게 하기 위해 대부분의 함수를 apicore.php 로 이동

----------------------------------------------------------------------------
* Version 0.9.8 (2006-07-19):
+ Fix
	- 첨부 방식을 바꿈. 파일의 해시값으로부터 파일명을 생성함.
	- Trunk에서 가져온 함수 이름 앞에 "_"을 두어 충돌을 회피함
	- metaWeblog.getCategories 호출에 rssUrl, description 을 추가함.

----------------------------------------------------------------------------
* Version 0.9.7 (2006-07-09):
+ New
    - MetaWeblog api에서 멀티미디어 파일 첨부가 가능하게됨.
	  (Zoundry, Semagic에서 테스트 됨)

+ Fix
	- getpost 메소드에서 두번의 escaping이 일어나는 것을 고침.
	- 글 삭제시 댓글 및 트랙백받은 것도 삭제.
	- 새 글을 올리거나 편집후 RSS에 반영되지 않는 것을 고침.
