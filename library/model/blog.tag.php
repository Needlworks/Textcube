...
function getTagId($blogid, $name) {
	...
	return Data_IAdapter::queryCell("SELECT id FROM {$database['prefix']}Tags WHERE name = '$name'");
}

function getTagById($blogid, $id) {
	global $database;
	return Data_IAdapter::queryCell("SELECT name FROM {$database['prefix']}Tags WHERE id = $id");
}
...
function getTags($blogid, $entry) {
	...
	if (doesHaveOwnership())
		$result = Data_IAdapter::query("SELECT t.* FROM `{$database['prefix']}Tags` t
		...
		$result = Data_IAdapter::query("SELECT t.* FROM `{$database['prefix']}Tags` t
			...
			ORDER BY t.name");
	if ($result) {
		while ($tag = Data_IAdapter::fetch($result))
			array_push($tags, $tag);
	}
	return $tags;
}
...
function getSiteTags($blogid) {
	global $database;
	if (doesHaveOwnership())
		$names = Data_IAdapter::queryAll("SELECT t.id,name FROM `{$database['prefix']}Tags` t, 
			`{$database['prefix']}TagRelations` r 
			WHERE t.id = r.tag AND r.blogid = $blogid 
			GROUP BY r.tag 
			ORDER BY t.name 
...
function getTagFrequencyRange() {
	...
	$max = $min = 0;
	if (doesHaveOwnership())
		$max = Data_IAdapter::queryCell("SELECT count(r.entry) `cnt` FROM `{$database['prefix']}TagRelations` r 
		...
		$max = Data_IAdapter::queryCell("SELECT count(r.entry) `cnt` FROM `{$database['prefix']}TagRelations` r
			...
			DESC LIMIT 1");
/*	if (doesHaveOwnership())
		$min = Data_IAdapter::queryCell("SELECT count(r.entry) `cnt` FROM `{$database['prefix']}TagRelations` r 
			WHERE r.blogid = $blogid 
			GROUP BY r.tag 
			ORDER BY `cnt` 
			LIMIT 1");
...
function getTagFrequency($tag, $max, $min) {
	...
	if (is_array($tag) && array_key_exists('cnt', $tag)) $count = $tag['cnt'];
		...
		if (doesHaveOwnership())
			$count = Data_IAdapter::queryCell("SELECT count(*) FROM `{$database['prefix']}Tags` t
				INNER JOIN `{$database['prefix']}TagRelations` r ON r.tag = t.id AND r.blogid = $blogid
				WHERE t.name = '" . Data_IAdapter::escapeString($tag['name']) . "'");
		else
			$count = Data_IAdapter::queryCell("SELECT count(*) FROM `{$database['prefix']}Tags` t
				INNER JOIN `{$database['prefix']}TagRelations` r ON r.tag = t.id AND r.blogid = $blogid 
...
function suggestLocalTags($blogid, $filter) {
	global $database;
	$tags = array();
	$result = Data_IAdapter::queryAll("SELECT DISTINCT t.name, count(*) cnt 
...
function addTagsWithEntryId($blogid, $entry, /*string array*/$taglist)
{
	...
	foreach($tmptaglist as $tag) {
		$tag = Data_IAdapter::escapeString(UTF8::lessenAsEncoding(trim($tag), 255));
		array_push($taglist, $tag);
	}

	// step 1. Insert Tags
	$tagliststr = '(\'' . implode('\') , (\'', $taglist) . '\')';
	Data_IAdapter::execute("INSERT IGNORE INTO {$database['prefix']}Tags (name) VALUES $tagliststr ");
	...
	$tagliststr =  '\'' . implode('\' , \'', $taglist) . '\'';
	/*
	Data_IAdapter::execute("INSERT INTO {$database['prefix']}TagRelations
	...
	*/
	...
	$tagIDs = Data_IAdapter::queryColumn("SELECT id FROM {$database['prefix']}Tags WHERE name in ( $tagliststr )");
	$tagrelations = array();
...
function modifyTagsWithEntryId($blogid, $entry, /*string array*/$taglist)
{
	...
	$taglist = array();
	foreach($tmptaglist as $tag) {
		$tag = Data_IAdapter::escapeString(trim($tag));
		array_push($taglist, $tag);
	}
	
	// step 1. Get deleted Tag
	$tmpoldtaglist = Data_IAdapter::queryColumn("SELECT name FROM {$database['prefix']}Tags
	...
	if (count($deletedTagList) > 0)
	{
		...
		$t1list = Data_IAdapter::queryColumn("SELECT id FROM {$database['prefix']}Tags WHERE name in ( $tagliststr )");
		if ($t1list === null) 
			return; // What?
		// Flushing pageCache
		...
		$taglist = Data_IAdapter::queryColumn(
	...
	// step 5. Delete Relation
		Data_IAdapter::execute("DELETE FROM {$database['prefix']}TagRelations WHERE blogid = $blogid AND entry = $entry AND tag in ( $tagliststr )");
	
	// step 6. Delete Tag
		$nottargets = Data_IAdapter::queryColumn("SELECT DISTINCT tag FROM {$database['prefix']}TagRelations WHERE tag in ( $tagliststr )");
...
{
	global $database;
	$taglist = Data_IAdapter::queryColumn("SELECT tag 
	...
	if ($taglist !== null) {
		...
		Data_IAdapter::execute("DELETE FROM {$database['prefix']}TagRelations 
			WHERE blogid = $blogid 
				AND entry = $entry");
		$nottargets = Data_IAdapter::queryColumn("SELECT DISTINCT tag 
			FROM {$database['prefix']}TagRelations 
			WHERE tag in ( $tagliststr )");
		if (count($nottargets) > 0) {
			$nottargetstr	= implode(', ', $nottargets);
			Data_IAdapter::execute("DELETE FROM {$database['prefix']}Tags 
...
function deleteTagById($blogid, $id) {
	...
	$result = Data_IAdapter::execute('DELETE FROM '.$database['prefix'].'TagRelations WHERE blogid = '.$blogid.' AND tag = '.$id);
	if (!$result) {
		return false;
	}

	$count = Data_IAdapter::queryCell('SELECT COUNT(*) FROM '.$database['prefix'].'TagRelations WHERE tag = '.$id);
	if (intval($count) > 0) {
		Data_IAdapter::execute('DELETE FROM '.$database['prefix'].'Tags WHERE id = '.$id);
	}
