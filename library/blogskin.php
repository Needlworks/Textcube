...
class Skin {
    ...
    function __construct($name, $previewMode = false) {
        ...
        if ($previewMode == true || ($context->getProperty('service.skincache') != true) || !$this->loadCache()) {
            ...
            $sval = str_replace('./', str_replace(ROOT, $context->getProperty('uri.service'), __TEXTCUBE_SKIN_DIR__) . "/$name/", $sval);

            $this->noneCommentMessage = str_replace('./', str_replace(ROOT, $context->getProperty('uri.service'), __TEXTCUBE_SKIN_DIR__) . "/$name/", $this->noneCommentMessage);
            $this->singleCommentMessage = str_replace('./', str_replace(ROOT, $context->getProperty('uri.service'), __TEXTCUBE_SKIN_DIR__) . "/$name/", $this->singleCommentMessage);
            $this->noneTrackbackMessage = str_replace('./', str_replace(ROOT, $context->getProperty('uri.service'), __TEXTCUBE_SKIN_DIR__) . "/$name/", $this->noneTrackbackMessage);
            $this->singleTrackbackMessage = str_replace('./', str_replace(ROOT, $context->getProperty('uri.service'), __TEXTCUBE_SKIN_DIR__) . "/$name/", $this->singleTrackbackMessage);
