...
class Skin {
	...
	function Skin($name, $previewMode = false) {
		...
		if($previewMode == true || ($service['skincache'] != true) || !$this->loadCache()) {
			$this->noneCommentMessage = Model_Setting::getBlogSettingGlobal('noneCommentMessage',null);
			$this->singleCommentMessage = Model_Setting::getBlogSettingGlobal('singleCommentMessage',null);
			$this->noneTrackbackMessage = Model_Setting::getBlogSettingGlobal('noneTrackbackMessage',null);
			$this->singleTrackbackMessage = Model_Setting::getBlogSettingGlobal('singleTrackbackMessage',null);
			...
			if (($name == '.') || ($name == '..')) {
				Utils_Respond::ErrorPage(_text('스킨 정보가 존재하지 않습니다.'), _text('로그인'), $blogURL."/owner");
			}
	...
	function applyMF2Entry( $type, $content ) {
		...
		if(Model_Setting::getBlogSettingGlobal('useMicroformat',3)>2) {
