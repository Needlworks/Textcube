...
class TrackbackLog {
	...
	function open($filter = '', $fields = '*', $sort = 'written') {
		...
		if (!empty($sort))
			$sort = 'ORDER BY ' . $sort;
		$this->close();
		$this->_result = POD::query("SELECT $fields FROM {$database['prefix']}TrackbackLogs WHERE blogid = ".getBlogId()." $filter $sort");
		if ($this->_result) {
	...
	function nextId($id = 0) {
		global $database;
		$maxId = POD::queryCell("SELECT max(id) FROM {$database['prefix']}TrackbackLogs WHERE blogid = ".getBlogId());
		if($id == 0)
			return $maxId + 1;
		else
